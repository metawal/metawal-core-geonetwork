...
a.unselect:a.select)(f).then(function(a){b.selectedRecordsCount=a[0];c["geonet:info"].selected=!0!==c["geonet:info"].selected})};b.selectAll=function(c){(c?a.selectAll:a.selectNone)().then(function(a){b.selectedRecordsCount=a[0];angular.forEach(b.searchResults.metadata,function(a){a["geonet:info"].selected=c?!0:!1})})};b.view=function(b,c){a.view(b)};b.edit=function(b,c){a.edit(b)}}}}])})();(function(){angular.module("gn_search_results",["gn_search_results_directive"])})();(function(){angular.module("gn_search_controller",[]).controller("GnSearchController",["$scope","$routeParams","$http","$rootScope","$translate","$compile","gnMetadataManagerService","gnSearchManagerService","gnUtilityService",function(a,b,c,d,e,g,f,m,k){a.searchRecordsFilter="";a.searchRecords=null;a.searchRecordsPagination={pages:-1,currentPage:0,hitsPerPage:20};searchFn=m.register({records:"searchRecords",filter:"searchRecordsFilter",pager:"searchRecordsPagination"},a);a.searchRecordsSearchFor=
function(b){a.searchRecordsFilter={any:b?b.target.value:""};a.searchRecordsPagination.currentPage=0;searchFn()};a.$watch("searchRecordsPagination.currentPage",function(){searchFn()})}])})();(function(){var a=angular.module("gn","ngRoute pascalprecht.translate gn_language_switcher gn_utility_service gn_search_manager gn_metadata_manager gn_search_results gn_pagination gn_cat_controller gn_search_controller".split(" "));a.constant("$LOCALES",["core"]);a.factory("localeLoader",["$http","$q",function(a,c){return function(d){var e=[];angular.forEach(d.locales,function(g,f){var m=d.prefix+d.key+"-"+g+d.suffix,k=c.defer();e.push(k.promise);a({method:"GET",url:m}).success(function(a,b,c,d){k.resolve(a)}).error(function(a,
b,c,e){k.reject(d.key)})});return c.all(e)}}]);a.config(["$translateProvider","$LOCALES",function(a,c){a.useLoader("localeLoader",{locales:c,prefix:"../../catalog/locales/",suffix:".json"});var d=location.href.split("/")[5].substring(0,2)||"en";a.preferredLanguage(d);moment.lang(d)}])})();(function(){var a=angular.module("gn_login_controller",[]);a.constant("$LOCALES",["core"]);a.controller("GnLoginController",["$scope","$http","$rootScope","$translate","$location","$window",function(a,c,d,e,g,f){a.registrationStatus=null;a.passwordReminderStatus=null;a.sendPassword=!1;a.password=null;a.passwordCheck=null;a.userToRemind=null;a.changeKey=null;a.passwordUpdated=!1;a.setSendPassword=function(c){a.sendPassword=c;$("#username").focus()};a.register=function(f){c.get("create.account@json?"+
$(f).serialize()).success(function(c){a.registrationStatus=c}).error(function(a){d.$broadcast("StatusUpdated",{title:e("registrationError"),error:a,timeout:0,type:"danger"})})};a.remindMyPassword=function(f){c.get("password.reminder@json?"+$(f).serialize()).success(function(c){a.passwordReminderStatus=c;a.sendPassword=!1}).error(function(a){d.$broadcast("StatusUpdated",{title:e("passwordReminderError"),error:a,timeout:0,type:"danger"})})};a.updatePassword=function(f){c.get("password.change@json?"+
$(f).serialize()).success(function(c){"null"==c&&(a.passwordUpdated=!0)}).error(function(a){d.$broadcast("StatusUpdated",{title:e("passwordUpdateError"),error:a,timeout:0,type:"danger"})})};(function(){-1!==f.location.pathname.indexOf("new.password")&&angular.forEach(f.location.search.replace("?","").split("&"),function(c){0===c.indexOf("username")?a.userToRemind=c.split("=")[1]:0===c.indexOf("changeKey")&&(a.changeKey=c.split("=")[1])})})()}])})();(function(){var a=angular.module("gn_login",["gn","gn_login_controller"]);a.constant("$LOCALES",["core"]);a.config(["$translateProvider","$LOCALES",function(a,c){a.useLoader("localeLoader",{locales:c,prefix:"../../catalog/locales/",suffix:".json"});var d=location.href.split("/")[5].substring(0,2)||"en";a.preferredLanguage(d);moment.lang(d)}])})();
